...
import ScreenshotTool from '@/components/ui/ScreenshotTool';
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') {
        ...
        else if (screenshotOpen) setScreenshotOpen(false);
    ...
    <div className="w-screen h-screen overflow-hidden bg-black relative select-none">
      ...
      {state.view === 'desktop' && (
        <>
          ...
          {screenshotOpen && <ScreenshotTool onClose={() => setScreenshotOpen(false)} />}
        </>
      )}
    </div>
  );
