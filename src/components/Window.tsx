...
export default function WindowComponent({ window: win }: Props) {
  const { dispatch, closeWindow, focusWindow, currentTheme, apps } = useOS();
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      if (isDragging) {
        const newX = Math.max(0, Math.min(e.clientX - dragOffset.x, window.innerWidth - win.size.width));
        const newY = Math.max(28, Math.min(e.clientY - dragOffset.y, window.innerHeight - win.size.height));
        dispatch({ type: 'MOVE_WINDOW', windowId: win.id, position: { x: newX, y: newY } });
      }
      if (isResizing) {
