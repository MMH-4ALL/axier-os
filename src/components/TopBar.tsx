...
import ThemesMenu from '@/components/ui/ThemesMenu';
...
export default function TopBar() {
  ...
  // Close themes menu on outside click
  useEffect(() => {
    if (!showThemes) return;
    const handler = (e: MouseEvent) => {
      if (!(e.target as Element)?.closest('.themes-trigger')) setShowThemes(false);
    };
    document.addEventListener('mousedown', handler);
    return () => document.removeEventListener('mousedown', handler);
  }, [showThemes]);
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative">
          <button
            className="themes-trigger flex items-center justify-center w-6 h-6 rounded-md opacity-70 hover:opacity-100 transition-opacity"
            style={{ color: currentTheme.colors.text }}
            onClick={() => setShowThemes(v => !v)}
            title="Change theme"
          >
            <div className="w-3.5 h-3.5 rounded-full" style={{ background: currentTheme.colors.accent }} />
          </button>
          {showThemes && <ThemesMenu onClose={() => setShowThemes(false)} />}
        </div>
